...
test('20b. Tenten normale ne reçoit pas le bonus Kenjutsu de Tenten Rikudo', () => assert.equal(calculateFinalStats(build({ kenjutsu: card('Tenten'), ninjutsu: card('Base', { ninjutsuAttack: 80 }) })).ninjutsuAttack, 80))
test('20c. Tenten Rikudo conserve le bonus Kenjutsu existant', () => assert.equal(calculateFinalStats(build({ kenjutsu: card('Tenten Rikudo'), ninjutsu: card('Base', { ninjutsuAttack: 80 }) })).ninjutsuAttack, 84))
...
test('28. Toneri KM en Kekkei Môra donne +10% Chakra et Ninjutsu', () => { const stats = calculateFinalStats(build({ 'kekkei-mora': card('Toneri KM'), chakra: card('C', { chakra: 80 }), ninjutsu: card('N', { ninjutsuAttack: 80, ninjutsuDefense: 80 }) })); assert.equal(stats.chakra, 88); assert.equal(stats.ninjutsuAttack, 88) })
