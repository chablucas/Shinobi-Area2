...
export async function requireAuth(request: Request, response: Response, next: NextFunction) {
  ...
  const user = await prisma.user.findUnique({ where: { id: (request as AuthenticatedRequest).userId }, select: { blockedAt: true } })
  if (!user) {
    response.status(401).json({ error: 'Token invalide ou expiré' })
    return
  }
  if (user.blockedAt) {
    response.status(403).json({ error: 'Votre accès au site a été bloqué par un administrateur.' })
    return
  }
  next()
}
